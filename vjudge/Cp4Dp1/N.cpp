...
void process(){
    ...
    vi dec(n, 0);
    FORN(i, n){
        auto it1 = lower_bound(seq1.begin(), seq1.end(), a[i]);
        int k = it1 - seq1.begin();
        inc[i] = k + 1;
        if(it1 == seq1.end()) seq1.push_back(a[i]); else *it1 = a[i];

        int rev = -a[i];
        auto it2 = lower_bound(seq2.begin(), seq2.end(), rev);
        int k2 = it2 - seq2.begin();
        dec[i] = k2 + 1;
        if(it2 == seq2.end()) seq2.push_back(rev); else *it2 = rev;
        
    }
    int ans = 0;
    FORN(i, n){
        maxi(ans, inc[i] + dec[i] - 1);
    }
    cout << ans << endl;
}
int32_t main() {
