...
void process(){
    ...
    FORE(i, m){
        int start = s[i];
        int loop = start + g[i] - 1 > n ? 1 : 0;
        int end = s[i] + g[i] - 1 - loop * n;

        int aPoint = preA[end] + loop * preA[n] - preA[start];
        int bPoint = preB[end] + loop * preB[n] - preB[start];

        int point = max(aPoint, bPoint);
        int isAWin = aPoint > bPoint;
        ...
        while(l <= r){
            ...
            int currentPoint = isAWin? preA[mid] : preB[mid];
